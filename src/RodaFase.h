	...
	do {
		do {
			...
			if(!enemyGen(fase, info.dificuldade, info.linhas+1))
				return 1;
			if(!initFase(fase, cor, extensao, extcor, conexao, concor, g.mapa, &info, redraw))
				return 1;
			vivo = rodaFase(fase, cor, extensao, extcor, conexao, concor, g, &pl, &score, info, &redraw);
			if(vivo == 2)
				return 1;
...
int faseCustom(TGraphs g) {
	...
	do {
		do {
			...
			redraw = 1;
			vivo = rodaFase(fase, cor, extensao, extcor, NULL, NULL, g, &p, &score, info, &redraw);
			if(vivo == 2)
				return 1;
		} while(vivo == -1);
